...
def get_data():
    data = pd.read_csv('./data_2017_2019_with_price.csv')
...
class Stock_Env:
    ...
    def reset(self):
        ...
        self.total_cost = 0
        if self.train:
            temp_time = np.random.randint(self.time_period, len(self.time)-252)
            ...
            self.totalday = temp_time
        self.totalday = temp_time
        temp = self.record.loc[self.time[self.timeid+1-self.time_period:self.timeid+1],self.codes_dict[self.code]*3+1:self.codes_dict[self.code]*3+3].values.reshape(1,-1)
    ...
    def step(self, action):
        ...
        while (self.rowid, self.code) not in self.data.index:
            ...
            if (self.timeid != len(self.time)-1):
                ...
                return np.zeros(self.time_period*3), 0, True
        if (self.timeid == len(self.time)-1):
            done = True
        ...
        return (self.record.loc[self.time[self.timeid+1-self.time_period:self.timeid+1], self.codes_dict[self.code]*3+1:self.codes_dict[self.code]*3+3].values.reshape(1,-1), reward, done)
